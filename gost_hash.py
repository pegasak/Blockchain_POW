...
Tau = tuple(i % 8 * 8 + i // 8 for i in range(64))
...
def PS(data):
    return bytearray(Pi[data[Tau[i]]] for i in range(BLOCKSIZE))
...
def g(n, h, m):
    return strxor(strxor(E(LPS(strxor(h[:8], pack("<Q", n)) + h[8:]), m), h), m)

def digest(digest_size, data):
    h = BLOCKSIZE * (b'\x01' if digest_size == 256 else b'\x00')
    chk = BLOCKSIZE * b'\x00'
    n = 0
    ...
    # Padding
    tail = data[len(data) // BLOCKSIZE * BLOCKSIZE:]
    padded = tail + b'\x01' + b'\x00' * (BLOCKSIZE - len(tail) - 1)
    h = g(n, h, padded)
    n += len(tail) * 8
    chk = add512bit(chk, padded)
    h = g(0, h, pack("<Q", n) + 56 * b'\x00')
    h = g(0, h, chk)

    return h[32:] if digest_size == 256 else h

def hexdigest(digest_size, data):
    return hexenc(digest(digest_size, data))

def generate_prng(seed_data: str, digest_size=256, count=10) -> list[bytes]:
    h0 = hexdigest(digest_size, seed_data.encode("utf-8"))
    h0_bytes = bytes.fromhex(h0)

    prng_output = []
    for i in range(1, count + 1):
        i_bytes = i.to_bytes(2, byteorder='big')
        input_data = h0_bytes + i_bytes
        hi = hexdigest(digest_size, input_data)
        hi_bytes = bytes.fromhex(hi)
        prng_output.append(hi_bytes)
